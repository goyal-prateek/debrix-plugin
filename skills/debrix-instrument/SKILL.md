...
Requires Python `>=3.11`. Package is alpha (`0.1.0a2`); APIs may change.
...
# ... run agent ...
force_flush()  # always call before short scripts exit (OTLP + payload uploads)
```
...
### Message / response payloads (opt-in)
...
### Capture modes (env / Debrix settings)

Not kwargs on `configure()` — read from env or desktop `settings.json`:

| Env | Values | Effect |
| --- | ------ | ------ |
| `DEBRIX_CAPTURE_MESSAGES` | `full` (default) \| `preview` \| `off` | Blob upload vs preview-only vs skip |
| `DEBRIX_MAX_PAYLOAD_BYTES` | default `209715200` | Over-cap → `capture_error`, no blob |
| `DEBRIX_PREVIEW_CHARS` | default `4096` | Head+tail preview budget |

`full` mode uploads bodies to `POST {endpoint}/v1/payloads` (gzip) and sets
`blob_ref` on the span. Without `force_flush()`, short scripts can lose payload
bodies even if the span landed.
...
| Path | What it shows |
| ---- | ------------- |
| `examples/basic-agent/run.py` | `configure(batch=False)` + `force_flush` |
| `examples/basic-agent/src/basic_agent/agent.py` | `@trace_agent(name="research_agent")` |
| `examples/basic-agent/src/basic_agent/tools.py` | `@trace_tool` |
| `examples/basic-agent/src/basic_agent/complete.py` | Canonical LLM span + message recording |
| `examples/basic-agent/fail_run.py` | Failed-trace demo for Observe / MCP |

## 6. Gotchas

1. **Desktop must be open** — OTLP `:17418` and `/v1/payloads`.
2. **`force_flush()` before exit** in scripts — payloads can lag behind spans.
